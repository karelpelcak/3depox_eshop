<script lang="ts">
    ...
    const FetchProductById = async (productId: number) => {
        try {
            const response = await fetch(`/api/products/productbyid?ProductId=${productId}`, {
                method: 'GET',
                headers: {
    ...
    $: totalProductPrice = productData ? productData.ProductPrice * ProductQuantity : 0;
</script>
...
{:else if productData !== null}
    ...
    >
        ...
        <div class="flex gap-4 md:gap-2 justify-center items-center">
            ...
            <button class="font-extrabold text-2xl" on:click={() => handleAddToCart(ProductId)}> + </button>
        </div>
        <h1 class="text-lg md:text-base">{totalProductPrice} Kč</h1>
    </div>
{:else}
    <div>Loading...</div>
{/if}
